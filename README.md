...
본 실습은 `LangGraph`를 활용하여 사용자의 질문을 단계적으로 처리하는 멀티에이전트 워크플로우를 구현한 예제입니다.

사용자의 질문에서 운동 추천에 필요한 증상 또는 고민 상태를 추출하고, 해당 상태를 해결하는 데 도움이 될 수 있는 운동 후보를 생성한 뒤, 최종적으로 사용자가 이해하기 쉬운 답변을 생성합니다.
...
본 실습은 단순히 LLM에 한 번 질문하고 답변을 받는 구조가 아니라, 여러 개의 역할 기반 에이전트를 `LangGraph`의 노드로 구성하여 순차적으로 실행하는 구조입니다.
...
본 실습의 핵심은 `AgentState`라는 상태값이 각 노드를 지나면서 점진적으로 채워진다는 점입니다.
...
|---|---|
| Python | 전체 코드 구현 언어 |
| Jupyter Notebook | 실습 코드 작성 및 실행 환경 |
...
│
├── README.md
├── langgraph_exercise_agent.ipynb
├── requirements.txt
├── graph_structure.png
└── result_capture.png
```
...
|---|---|
| `README.md` | 실습 설명 문서 |
| `langgraph_exercise_agent.ipynb` | LangGraph 멀티에이전트 실습 노트북 |
...
본 실습에서는 Ollama를 통해 로컬 LLM 모델을 실행합니다.
...
본 실습에서는 `TypedDict`를 사용하여 상태 구조를 정의했습니다.
...
아래 이미지는 Jupyter Notebook 환경에서 LangGraph 워크플로우를 실행한 결과입니다.

<p align="center">
  <img src="result_capture.png" alt="LangGraph 실행 결과" width="900">
</p>

실행 결과는 다음과 같은 순서로 출력됩니다.
...
3. 최종 응답
   - 추출된 증상과 운동 후보를 바탕으로 LG AI 모델이 운동별 추천 이유를 생성
```

최종 응답은 사용한 LLM 모델의 생성 결과이므로, 문장 구성이나 표현 방식은 모델 응답 특성에 따라 달라질 수 있습니다.
...
### 13.3 Jupyter Notebook 실행

```bash
jupyter notebook
```

이후 `langgraph_exercise_agent.ipynb` 파일을 열고 셀을 순서대로 실행합니다.

---

## 14. requirements.txt

본 실습 실행에 필요한 라이브러리는 다음과 같습니다.
...
본 실습은 LangGraph를 활용하여 LLM 기반 멀티에이전트 구조를 직접 구성한 예제입니다.
...
본 실습은 LangGraph와 LLM 기반 멀티에이전트 구조를 학습하기 위한 예제입니다.
